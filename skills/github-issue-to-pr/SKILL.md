...
version: 0.3.0
author: the Protean publication, Hermes Agent
license: MIT
...
Reproduce the bug or demonstrate the missing behavior on the current default branch with a failing test or fixture, using `search_files` and `read_file` to trace the reported path. Then check the second question: is the "bug" actually deliberate design? Run `git log -p -S "<symbol>"` on the code the issue wants changed and read the original commit's intent — a missing link or restriction is often the feature. Challenge stale or flawed issue prose instead of implementing it blindly. Then clear a defect bar and write down which one: a user-visible failure (duplicate execution, wrong or lost data, a crash), a written invariant violated (name the document and quote the line), or unintended behaviour proven (a surface requests a resource nothing needs). Design intent lives in prose as well as history — read the area's `AGENTS.md` and the user docs, including any "do not fix this" list, and quote the line that settles it. Process and topology findings are the trap: an extra process, an extra connection, or two holders of one file is usually the architecture, and the mechanism is not the harm, so state the mechanism, the failure it causes, and the falsifier that would show the behaviour is intended. Done when the root cause or feature gap is demonstrated in current code, the defect bar is named, and the change doesn't fight an intentional design — and when the change rests on a design decision rather than a defect, it goes to the maintainers as a proposal instead of a patch.
...
Run the formatter, lint, typecheck, and the repo's canonical test entrypoint on affected areas; use `requesting-code-review` on the diff. **If the change alters runtime behavior — process spawn/reap, file writes, routing, lifecycle — also prove it on the running system before you push** (`github-pr-workflow` §2b): rebuild the artifact the real entry point loads, reproduce the original symptom, and capture the process tree, ports, and `lsof` evidence that shows it gone. Unit tests are not this proof. Then push and open the PR right away — the PR is what dispatches CI, and CI latency is the long pole; do not sit on finished work. Load `github-pr-workflow` for PR mechanics: conventional branch/commit, body linking the issue with problem, approach, tests, risk, and exclusions. Read the PR back and verify head SHA, base, title, and files. Done when the PR exists with the intended diff and CI is running.
...
- Filing or fixing a documented topology as a defect: an extra process, an extra connection, or two holders of one file clears no defect bar on its own.
...
- [ ] Premise reproduced on current code; design intent checked via git history AND the area's `AGENTS.md`/docs; defect bar named.
